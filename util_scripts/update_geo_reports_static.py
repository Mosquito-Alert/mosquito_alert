...
from mosquito_alert.identification_tasks.models import IdentificationTask  # noqa: E402
from mosquito_alert.reports.models import Report  # noqa: E402
...
def qs_to_local_csv(qs, filename, path=settings.STATIC_ROOT, fields=None):
    ...
    if not isinstance(fields, dict):
        raise ValueError(
            "fields must be a dictionary mapping CSV column names to callables."
        )
...
if __name__ == "__main__":
    ...
    qs_to_local_csv(
        ...
        fields={
            ...
            "lon": lambda x: x.point.x,
            "lat": lambda x: x.point.y,
            "species": lambda x: x.identification_task.taxon.name.lower().replace(
                " ", "_"
            ),
        },
    )
