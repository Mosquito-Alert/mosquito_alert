var MapView = MapView.extend({
        ...
        'complete': function(resp) {
          ...
          var probabilities = new L.geoJson(prob, {
            'style': function(feature) {
              let range = _this.getForecastRange(layer.prob_ranges, feature.properties.prob);
              return {
          ...
          MOSQUITO.app.mapView.layers.standard_deviation = new L.geoJson(sd, {
            pointToLayer: function (feature, latlng) {
              let range = _this.getForecastRange(layer.sd_ranges, feature.properties.val);
    ...
    loadForecastModel: function() {
      ...
      this.forecast_layer = L.canvasTiles()
          .params({ debug: false, padding: 0 })
          .drawing(this.prob_drawingOnCanvas);

      this.forecast_layer.addTo(this.map);

      this.forecast_layer_sd = L.canvasTiles()
        .params({ debug: false, padding: 1256 })
        .drawing(this.sd_drawingOnCanvas);

      this.forecast_layer_sd.addTo(this.map);
    },
    ...
    refreshForecastModel: function() {
      ...
      this.map.removeLayer(this.forecast_layer_sd);
      $.ajax({
        'url': MOSQUITO.config.URL_API + 'get/pred/data/' + date,
        'context': this,
        'complete': function(resp) {
          ...
          // STANDRARD DEVIATION
          MOSQUITO.app.tileIndex_sd = geojsonvt(all_data.sd, this.userFixtileOptions);
          this.map.addLayer(this.forecast_layer_sd);
        }
      })
    },
    ...
    sd_drawingOnCanvas: function(canvasOverlay, params) {
      ...
      let tileIndex = MOSQUITO.app.tileIndex_sd;
      var ctx = params.canvas.getContext('2d');
      ctx.globalCompositeOperation = 'source-over';
      ...
      for (var i = 0; i < features.length; i+=1) {
        if (typeof features[i].tags.color === 'undefined') {
          let range = _this.getForecastRange(layer.sd_ranges, features[i].tags.val);
          features[i].tags.color = range.color;
        }
        ctx.fillStyle = features[i].tags.color;

        var d = features[i].geometry[0];
        var x = d[0] / extent * 256;
        var y = d[1] / extent * 256;
        ctx.beginPath();
