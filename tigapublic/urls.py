...
urlpatterns = patterns('',
    ...
    url(r'^reports/([0-9.,-]+)/(all|[0-9.,-]+)/(all|[0-9.,]+)/([a-zA-Z,_]+)/([0|1|2])/(none|NONE|[#0-9a-zA-Z]+)/$', views.reports_notif),#this should go first
    #url(r'^reports/([0-9.,-]+)/(.*)/(.*)/(.*)/$', views.reports),
    url(r'^reports/([0-9.,-]+)/(all|[0-9.,-]+)/(all|[0-9.,]+)/([a-zA-Z,_]+)/[nN]/(none|NONE|[#0-9a-zA-Z]+)/$', views.reports),
    ...
    url(r'^quarta/$', views.getStormDrainData),
