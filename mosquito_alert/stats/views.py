...
def show_usage(request):
    def get_cumsum(qs, datetime_fieldname: str, trunc_by: str) -> dict:
        qs = (
            qs.annotate(
                week_epoch=Extract(Trunc(datetime_fieldname, kind=trunc_by), "epoch")
            )
            .values("week_epoch")
            .annotate(n=Count(1))
            .order_by("week_epoch")
        ...
        for obj in qs.iterator():
            cumsum += obj["n"]
            result.append({"date": obj["week_epoch"], "n": cumsum})
        return result

    context = {
        "users": get_cumsum(
            qs=TigaUser.objects, datetime_fieldname="registration_time", trunc_by="week"
        ),
...
def workload_pending_per_user(request):
    if request.method == "GET":
        ...
        pending_detail = []
        for ano in user.userstat.pending_annotations.select_related(
            "identification_task", "identification_task__report"
        ).iterator():
            pending_detail.append(
                {
                    "report_id": ano.identification_task.report.pk,
                    "created": ano.identification_task.report.creation_time.strftime(
                        "%d/%m/%Y - %H:%M:%S"
                    ),
                }
            )

        last_activity = user.userstat.completed_annotations.order_by(
            "last_modified"
        ).last()
        ...
        pending = {
            ...
            "last_activity": last_activity.last_modified.strftime("%d/%m/%Y")
            if last_activity
            else "Never",
        }
...
def workload_stats_per_user(request):
    if request.method == "GET":
        ...
        qs = (
            user.userstat.completed_annotations.annotate(
        ...
        return Response(
            [(r["week_epoch"].timestamp() * 1000, r["n"]) for r in qs.iterator()]
        )
...
def workload_daily_report_input(request):
    if request.method == "GET":
        qs = (
            Report.objects.annotate(day_epoch=Trunc("server_upload_time", kind="day"))
            .values("day_epoch")
            .annotate(n=Count(1))
            .order_by("day_epoch")
        )
        ...
        # NOTE: convert to miliseconds (epoch)
        return Response(
            [(r["day_epoch"].timestamp() * 1000, r["n"]) for r in qs.iterator()]
        )
...
def workload_available_reports(request):
    if request.method == "GET":
        ...
        overall_pending = ExpertReportAnnotation.objects.filter(
            user__pk__in=user_id_filter, identification_task__isnull=False
        ).completed(False)

        data = {
            "current_pending_n": current_pending.count(),
            "current_progress_n": current_progress.count(),
            "overall_pending": overall_pending.count(),
        }
...
def stats_user_score(request, user_uuid=None):
    ...
    context = {
        "score_data": user.score_v2_struct,
        "score_last_update": user.last_score_update,
    }
...
def stats_user_ranking(request, page=1, user_uuid=None):
    ...
    if seek == "t":
        ...
        return HttpResponseRedirect(
            reverse("stats_user_ranking", args=[int(page_of_index), user_uuid])
        )
    else:
...
def global_assignments(request):
    ...
    n_unassigned = (
        task_qs.new()
        .in_exclusivity_period(False)
        .annotate(country=models.F("report__country"))
        .values("country")
        .annotate(total_count=models.Count(1))
        .values("country", "total_count")
    )

    n_in_exclusivity_period = (
        task_qs.in_exclusivity_period()
        .annotate(country=models.F("report__country"))
        .values("country")
        .annotate(total_count=models.Count(1))
        .values("country", "total_count")
    ...
    for data_list, fieldname in [
        (n_unassigned, "unassigned"),
        (n_progress, "progress"),
        (n_pending, "pending"),
        (n_blocked, "blocked"),
        (n_in_exclusivity_period, "reserved"),
    ]:
...
def global_assignments_list(request, country_code=None, status=None):
    ...
    for obj in (
        ...
        .prefetch_related("expert_report_annotations")
        .iterator(chunk_size=1000)
    ):
        names = []
        for annotation in obj.expert_report_annotations.all():
            names.append(annotation.user.get_full_name() or annotation.user.username)
