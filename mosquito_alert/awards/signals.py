    ...
    if created:
        try:
            ...
            if instance.type == Report.TYPE_ADULT or instance.type == Report.TYPE_SITE:
                ...
                awards = (
                    Award.objects.filter(given_to=instance.user)
                    .filter(report__creation_time__year=current_year)
                    .filter(category__category_label="start_of_season")
                )
                ...
                awards = (
                    Award.objects.filter(report__creation_time__year=report_year)
                    ...
                    .filter(category__category_label="daily_participation")
                ...
                report_before_this_one = (
                    Report.objects.filter(user=instance.user)
                    .filter(creation_time__lte=date_1_day_before_report_adjusted)
                    .order_by("-creation_time")
                    .first()
                )  # first is most recent
                if report_before_this_one is not None and one_day_between_and_same_week(
                    report_before_this_one.creation_time, instance.creation_time
                ):
                    ...
                    if (
                        ...
                        .filter(category__category_label="fidelity_day_2")
                        .count()
                        == 0
                        and Award.objects.filter(report=report_before_this_one)
                        .filter(category__category_label="fidelity_day_3")
                        .count()
                        == 0
                    ):
