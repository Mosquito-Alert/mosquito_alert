...
def send_new_award_notification(award: "Award") -> None:
    ...
    if user.locale:
        user_language_code = (
            user.locale if user.locale in dict(settings.LANGUAGES) else "en"
        )
    ...
    if report:
        if (
            ...
            and report.package_version
            <= settings.MINIMUM_PACKAGE_VERSION_SCORING_NOTIFICATIONS
        ):
            return
    ...
    # Add XP earning
    award_xp = (
        award.special_award_xp
        if award.special_award_xp
        else (award.category.xp_points if award.category else 0)
    )
    context_en["amount_awarded"] = context_native["amount_awarded"] = award_xp

    reason_awarded_txt = (
        award.special_award_text
        if award.special_award_text
        else (award.category.category_label if award.category else 0)
    )
    ...
    def render_body_html(template_lang, context):
        try:
            result_template = render_to_string(
                f"awards/award_notification_{template_lang}.html", context
            )
        except TemplateDoesNotExist:
            result_template = render_to_string(
                "awards/award_notification_en.html", context
            )

        return (
            result_template.replace("&amp;", "&")
