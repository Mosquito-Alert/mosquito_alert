...
def is_bite_report_followed(report):
    return report.responses.filter(
        question_id=MOSQUITO_BITE_QUESTION_ID, answer_id=MOSQUITO_BITE_ANSWER_ID
    ).exists()


def is_mosquito_report_followed(report):
    return report.responses.filter(
        question_id=BREEDING_SITE_MOSQUITO_QUESTION_ID,
        answer_id=BREEDING_SITE_MOSQUITO_ANSWER_ID,
    ).exists()
...
def get_site_report_score(report, result):
    ...
    if report.n_visible_photos > 0:
        ...
        local_result["awards"].append(
            {
                "reason_untranslated": "picture",
                "xp_awarded": BREEDING_SITE_PICTURE_REWARD,
            }
        )
    ...
    if report.located:
        ...
        local_result["awards"].append(
            {
                "reason_untranslated": "location",
                "xp_awarded": BREEDING_SITE_LOCATION_REWARD,
            }
        )
    ...
    if is_storm_drain(report):
        ...
        local_result["awards"].append(
            {
                "reason_untranslated": "storm_drain",
                "xp_awarded": SITE_STORM_DRAIN_REWARD,
            }
        )
    ...
    if is_water_answered(report):
        ...
        local_result["awards"].append(
            {
                "reason_untranslated": "water_question",
                "xp_awarded": SITE_WATER_QUESTION_REWARD,
            }
        )
    ...
    if is_mosquito_report_followed(report):
        ...
        local_result["awards"].append(
            {
                "reason_untranslated": "mosquito_report_follows_breeding_site",
    ...
    for award in report.report_award.all():
        if award.category:
            ...
            local_result["awards"].append(
                {
                    "reason_untranslated": award.category.category_label,
...
def get_adult_report_score(report, result):
    ...
    if identification_task and identification_task.taxon_id in [
        10,
        112,
        113,
        114,
        115,
    ]:  # Is culex or target aedes
        if picture:
            # local_result['awards'].append({"reason_untranslated": "picture", "reason": _("picture"), "xp_awarded": MOSQUITO_PICTURE_REWARD})
            local_result["awards"].append(
                {
                    "reason_untranslated": "picture",
                    "xp_awarded": MOSQUITO_PICTURE_REWARD,
                }
            )
        ...
        if report.located:
            ...
            local_result["awards"].append(
                {
                    "reason_untranslated": "location",
                    "xp_awarded": MOSQUITO_LOCATION_REWARD,
                }
            )
        ...
        if is_thorax_answered(report):
            ...
            local_result["awards"].append(
                {
                    "reason_untranslated": "thorax_question",
                    "xp_awarded": MOSQUITO_THORAX_QUESTION_REWARD,
                }
            )
        ...
        if is_abdomen_answered(report):
            ...
            local_result["awards"].append(
                {
                    "reason_untranslated": "abdomen_question",
                    "xp_awarded": MOSQUITO_ABDOMEN_QUESTION_REWARD,
                }
            )
        ...
        if is_leg_answered(report):
            ...
            local_result["awards"].append(
                {
                    "reason_untranslated": "leg_question",
                    "xp_awarded": MOSQUITO_LEG_QUESTION_REWARD,
                }
            )
    ...
    if is_bite_report_followed(report):
        ...
        local_result["awards"].append(
            {
                "reason_untranslated": "bite report follow",
                "xp_awarded": MOSQUITO_BITE_ANSWER_REWARD,
            }
        )
...
def get_unrelated_awards_score(user_uuid, user_uuids):
    ...
    if user_uuids is None:
        special_awards = Award.objects.filter(report__isnull=True).filter(
            given_to=user_uuid
        )
    else:
        special_awards = Award.objects.filter(report__isnull=True).filter(
            given_to__in=user_uuids
        )
    for award in special_awards:
        if award.category is None:
            ...
            awards.append(
                {
                    "reason_untranslated": award.special_award_text,
...
def compute_user_score_in_xp_v2(user_uuid):
    ...
    overall_df = pd.DataFrame(
        list(qs_overall.values_list("score_v2", "user_UUID")),
        columns=["score_v2", "user_UUID"],
    )
    adult_df = pd.DataFrame(
        list(qs_adult.values_list("score_v2_adult", "user_UUID")),
        columns=["score_v2_adult", "user_UUID"],
    )
    site_df = pd.DataFrame(
        list(qs_site.values_list("score_v2_site", "user_UUID")),
        columns=["score_v2_site", "user_UUID"],
    )
    ...
    overall_sorted_df["rank"] = overall_sorted_df["score_v2"].rank(
        method="dense", ascending=False
    )

    adult_sorted_df = adult_df.sort_values("score_v2_adult", inplace=False)
    adult_sorted_df["rank"] = adult_sorted_df["score_v2_adult"].rank(
        method="dense", ascending=False
    )

    site_sorted_df = site_df.sort_values("score_v2_site", inplace=False)
    site_sorted_df["rank"] = site_sorted_df["score_v2_site"].rank(
        method="dense", ascending=False
    )
    ...
    user_reports = Report.objects.filter(user__user_UUID=user_uuid).order_by(
        "-creation_time"
    )
    ...
    for report in adult_last_versions.iterator(chunk_size=1000):
        ...
        result["score_detail"]["adult"]["score"] += result["score_detail"]["adult"][
            "score_items"
        ][index]["report_score"]
        adult_score += result["score_detail"]["adult"]["score_items"][index][
            "report_score"
        ]
    ...
    for report in site_last_versions.iterator(chunk_size=1000):
        ...
        result["score_detail"]["site"]["score"] += result["score_detail"]["site"][
            "score_items"
        ][index]["report_score"]
        site_score += result["score_detail"]["site"]["score_items"][index][
            "report_score"
        ]
    ...
    overall_class = get_user_class(
        min_max_overall["max"], min_max_overall["min"], result["total_score"]
    )
    result["overall_class_value"] = overall_class["value"]
    result["overall_class_label"] = overall_class["label"]

    adult_class = get_user_class(
        min_max_adult["max"],
        min_max_adult["min"],
        result["score_detail"]["adult"]["score"],
    )
    ...
    site_class = get_user_class(
        min_max_site["max"],
        min_max_site["min"],
        result["score_detail"]["site"]["score"],
    )
    ...
    overall_number_below_rank = overall_sorted_df[
        overall_sorted_df["rank"] <= result["overall_rank_value"]
    ].count()["rank"]
    overall_number_total = overall_sorted_df.count()["rank"]

    adult_number_below_rank = adult_sorted_df[
        adult_sorted_df["rank"] <= result["score_detail"]["adult"]["rank_value"]
    ].count()["rank"]
    adult_number_total = adult_sorted_df.count()["rank"]

    site_number_below_rank = site_sorted_df[
        site_sorted_df["rank"] <= result["score_detail"]["site"]["rank_value"]
    ].count()["rank"]
    ...
    if overall_number_total == 0:
        ...
        result["overall_top_perc"] = (
            float(overall_number_below_rank) / float(overall_number_total)
        ) * 100.0
    ...
    if adult_number_below_rank == 0 and adult_number_total == 0:
        ...
        result["score_detail"]["adult"]["top_perc"] = (
            float(adult_number_below_rank) / float(adult_number_total)
        ) * 100.0
    ...
    if site_number_below_rank == 0 and site_number_total == 0:
        ...
        result["score_detail"]["site"]["top_perc"] = (
            float(site_number_below_rank) / float(site_number_total)
        ) * 100.0
