...
class Session(models.Model):
    id = models.AutoField(
        primary_key=True,
        help_text="Unique identifier of the session. Automatically generated by server when session created.",
    )
    session_ID = models.IntegerField(
        db_index=True,
        help_text="The session ID number. Should be an integer that increments by 1 for each session from a given user.",
    )
    ...
    session_start_time = models.DateTimeField(
        help_text='Date and time on phone when the session was started. Format as ECMA 262 date time string (e.g. "2014-05-17T12:34:56.123+01:00".'
    )
    session_end_time = models.DateTimeField(
        null=True,
        blank=True,
        help_text='Date and time on phone when the session was ended. Format as ECMA 262 date time string (e.g. "2014-05-17T12:34:56.123+01:00".',
    )
