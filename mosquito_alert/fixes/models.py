...
class Fix(TimeZoneModelMixin, models.Model):
    ...
    user_coverage_uuid = models.CharField(
        blank=True,
        null=True,
        max_length=36,
    ...
    fix_time = models.DateTimeField(
        help_text="Date and time when fix was recorded on phone. Format as ECMA "
        '262 date time string (e.g. "2014-05-17T12:34:56'
        '.123+01:00".'
    )
    server_upload_time = models.DateTimeField(
        auto_now_add=True,
        help_text="Date and time registered by server when "
        "it received fix upload. Automatically "
        "generated by server.",
    )
    phone_upload_time = models.DateTimeField(
        help_text="Date and time on phone when it uploaded fix. Format "
        "as ECMA "
        '262 date time string (e.g. "2014-05-17T12:34:56'
        '.123+01:00".'
    )
    masked_lon = models.FloatField(
        help_text=f"Longitude rounded down to nearest {GRID_SIZE} decimal degree."
    )
    masked_lat = models.FloatField(
        help_text=f"Latitude rounded down to nearest {GRID_SIZE} decimal degree."
    )
    mask_size = models.FloatField(
        null=True, blank=True, help_text="size of location mask used"
    )
    power = models.FloatField(
        null=True,
        blank=True,
        help_text="Power level of phone at time fix recorded, "
    ...
    def save(self, *args, **kwargs):
        ...
        self.masked_lon = round(
            floor(self.masked_lon / self.GRID_SIZE) * self.GRID_SIZE, 5
        )
        self.masked_lat = round(
            floor(self.masked_lat / self.GRID_SIZE) * self.GRID_SIZE, 5
        )
