...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        identification_tasks_qs = IdentificationTask.objects.all().select_related(
            "report", "taxon"
        )
        ...
        if options["multiprocessing"]:
            ...
            tasks_data = (
                (task, options["force"]) for task in identification_tasks_qs.iterator()
            )
            ...
            with Pool(processes=num_processors) as pool:
                ...
                list(
                    tqdm(
                        pool.imap_unordered(refresh_task, tasks_data, chunksize=100),
                        total=identification_tasks_qs.count(),
                    )
                )
        else:
            # If multiprocessing is not enabled, handle tasks without it
            for task in tqdm(
                identification_tasks_qs.iterator(),
                total=identification_tasks_qs.count(),
            ):
                try:
