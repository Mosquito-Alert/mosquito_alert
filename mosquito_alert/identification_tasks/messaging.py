...
def create_notification_content_finished_validation(identification_task: IdentificationTask, commit: bool = True) -> NotificationContent:
    ...
    def render_body_html(template_lang, context):
        try:
            return render_to_string(f'identification_tasks/validation_message_template_{template_lang}.html', context).replace('&amp;', '&')
        except TemplateDoesNotExist:
            return render_to_string('identification_tasks/validation_message_template_en.html', context).replace('&amp;', '&')
