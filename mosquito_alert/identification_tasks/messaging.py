...
def create_notification_content_finished_validation(
    identification_task: IdentificationTask, commit: bool = True
) -> NotificationContent:
    ...
    if report_user.locale:
        user_language_code = (
            report_user.locale
            if report_user.locale in dict(settings.LANGUAGES)
            else "en"
        )
    ...
    if identification_task.public_note:
        context_en["expert_note"] = context_native["expert_note"] = (
            identification_task.public_note
        )

    # Include any specific message for the user
    if identification_task.message_for_user:
        context_en["message_for_user"] = context_native["message_for_user"] = (
            identification_task.message_for_user
        )
    ...
    def render_body_html(template_lang, context):
        try:
            return render_to_string(
                f"identification_tasks/validation_message_template_{template_lang}.html",
                context,
            ).replace("&amp;", "&")
        except TemplateDoesNotExist:
            return render_to_string(
                "identification_tasks/validation_message_template_en.html", context
            ).replace("&amp;", "&")
    ...
    if taxon_name:
        context_native["validation_category"] = "{} ({})".format(
            taxon_name, identification_task.confidence_label
        )
    ...
    if taxon_name:
        context_en["validation_category"] = "{} ({})".format(
            taxon_name, identification_task.confidence_label
        )
...
def send_finished_identification_task_notification(
    identification_task: IdentificationTask, from_user: Optional[User] = None
) -> None:
