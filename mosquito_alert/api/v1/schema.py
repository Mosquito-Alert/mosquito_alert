...
class PointFieldExtension(OpenApiSerializerFieldExtension):
    ...
    def map_serializer_field(self, auto_schema, direction):
        openapi_type = (
            OpenApiTypes.STR if self.target.str_points else OpenApiTypes.FLOAT
        )
...
class WritableSerializerMethodField(OpenApiSerializerFieldExtension):
    ...
    def map_serializer_field(self, auto_schema, direction):
        return auto_schema._map_serializer_field(
            field=self.target.deserializer_field
            if direction == "request"
            else self.target.serializer_field,
            direction=direction,
