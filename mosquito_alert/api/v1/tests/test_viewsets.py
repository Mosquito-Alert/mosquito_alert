...
class AppUserJWTAuthenticationTestMixin:
    def test_unauth_access_must_return_401(self):
        ...
        assert response.headers[
            "WWW-Authenticate"
        ] == JWTAuthentication().authenticate_header(request)
...
class NonAppUserSessionAuthenticationTestMixin:
    ...
    def test_session_auth_csrf(
        self, non_app_api_client, user, method, include_csrf, expected_status_code
    ):
...
class TestGenericViewSet(
    AppUserJWTAuthenticationTestMixin,
    TokenAuthenticationTestMixin,
    NonAppUserSessionAuthenticationTestMixin,
    BaseTestGenericViewSet,
):
...
@pytest.mark.django_db
class TestGenericMobileOnlyViewSet(
    AppUserJWTAuthenticationTestMixin, BaseTestGenericViewSet
):
...
@pytest.mark.django_db
class TestGenericNoMobileViewSet(
    NonAppUserSessionAuthenticationTestMixin,
    TokenAuthenticationTestMixin,
    BaseTestGenericViewSet,
):
