...
class BaseReportTest:
    ...
    def test_package_is_set_on_report_create(self, app_user, data_create_request):
        ...
        app_api_client = AppAPIClient(
            device=Device.objects.create(
                ...
                mobile_app=MobileApp.objects.create(
                    package_name="testapp", package_version="1.2.3+4567"
                ),
            )
        )
        ...
        assert (
            report.package_version == 102
        )  # NOTE: only major and minor version are considered.
    ...
    def test_delete_method_performs_soft_delete(self, app_api_client, report_object):
        # Delete
        response = app_api_client.delete(self.endpoint + f"{report_object.pk}/")
    ...
    def test_user_can_see_note_if_owner(
        self, app_user, app_api_client, report_object, is_owner
    ):
        ...
        response = app_api_client.get(self.endpoint + f"{report_object.pk}/")
    ...
    def test_filter_boundary_uuid(
        self, app_api_client, report_object, use_test_cache_backend
    ):
        boundary = create_boundary_contains_point(point=report_object.point)
        response = app_api_client.get(self.endpoint + f"?boundary_uuid={boundary.uuid}")
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_filter_boundary_uuid_raise_400_if_boundary_not_exist(self, app_api_client):
        response = app_api_client.get(
            self.endpoint + "?boundary_uuid=00000000-0000-0000-0000-000000000000"
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "",
                lambda response: response.data["results"][0]["location"]["point"][
                    "latitude"
                ],
                lambda response: response.data["results"][0]["location"]["point"][
                    "longitude"
    ...
    def test_set_geo_precision_query_param(
        self,
        app_api_client,
        report_object,
        endpoint_suffix,
    ...
    def test_geo_json_response_schema_type(self, app_api_client):
        response = app_api_client.get(
            self.endpoint + "geo/", headers={"Accept": "application/geo+json"}
        )
        assert response.status_code == status.HTTP_200_OK
...
class TestObservationAPI(BaseReportTest):
    ...
    def test_observation_has_identification_only_if_published(
        self, app_api_client, is_published, identification_task
    ):
        observation = identification_task.report
        observation.published_at = timezone.now() if is_published else None
        observation.save()

        response = app_api_client.get(self.endpoint + f"{observation.pk}/")
        assert response.status_code == status.HTTP_200_OK
...
class TestIdentificationTaskAPI:
    ...
    def test_result(
        ...
        assert response.status_code == status.HTTP_200_OK
        assert (response.data["result"] is None) == expect_null

    def test_result_characteristics(
        self, api_client, permitted_user, identification_task
    ):
...
class TestIdentificationTaskPredictionAPI:
    ...
    def test_is_decisive_False_does_not_set_identification_task(
        self, api_client, identification_task, is_decisive
    ):
        ...
        response = api_client.patch(
            self.build_url(
                identification_task=identification_task, photo_uuid=photo.uuid
            ),
            data={"is_decisive": is_decisive},
            format="json",
        )
        assert response.status_code == status.HTTP_200_OK
...
class TestTokenAPI:
    ...
    @pytest.fixture(
        params=[pytest.lazy_fixture("user"), pytest.lazy_fixture("app_user")]
    )
    def any_user(self, request):
        return request.param

    @time_machine.travel("2024-01-01 00:00:00", tick=False)
    def test_user_last_login_is_updated_on_token_create(
        self, any_user, user_password, client
    ):
    ...
    def test_jwt_token_includes_user_type_on_obtain(
        self, any_user, user_password, client
    ):
        response = client.post(
            self.endpoint,
        ...
        decoded = jwt.decode(
            token,
            algorithms=["HS256"],
            key="dummy_secretkey",  # get from config.settings.test.SECRET_KEY
        )
        assert "user_type" in decoded
        expected_user_type = (
            "mobile_only" if isinstance(any_user, TigaUser) else "regular"
        )
    ...
    def test_jwt_token_includes_user_type_on_refresh(
        self, any_user, user_password, client
    ):
        token_class = import_string(api_settings.TOKEN_OBTAIN_SERIALIZER).token_class
    ...
    def test_jwt_token_includes_device_id_only_for_tigauser(
        self, any_user, user_password, client
    ):
        response = client.post(
            self.endpoint,
    ...
    @pytest.mark.parametrize("extra_data", [{}, {"device_id": ""}])
    ...
    @time_machine.travel("2024-01-01 00:00:00", tick=False)
    def test_device_is_created_if_not_exist_on_login(
        self, any_user, user_password, client
    ):
        assert Device.objects.all().count() == 0
    ...
    def test_device_last_login_is_updated_on_token_create(
        self, app_user, user_password, client
    ):
        device = Device.objects.create(user=app_user, device_id="unique_id_for_device")
    ...
    def test_jwt_token_includes_device_id_after_refresh(
        self, any_user, user_password, client
    ):
        response = client.post(
            self.endpoint,
    ...
    def test_device_last_login_is_not_updated_on_token_refresh(
        self, app_user, user_password, client
    ):
        device = Device.objects.create(user=app_user, device_id="unique_id_for_device")
    ...
    def test_device_is_set_to_logged_in_on_login_if_not_logged(
        self, app_user, user_password, client
    ):
        device = Device.objects.create(
            user=app_user,
    ...
    def test_token_verify_return_401_after_expiration(
        self, any_user, client, token_field
    ):
        with time_machine.travel("2024-01-01 00:00:00", tick=False) as traveller:
            token_class = import_string(
                api_settings.TOKEN_OBTAIN_SERIALIZER
            ).token_class
            token = token_class.for_user(any_user)
...
class TestDeviceAPI:
    ...
    def test_device_is_set_to_inactive_if_new_duplicated_device_is_created(
        ...
        response = app_api_client.post(
            ...
            data={
                ...
                "model": "another_model",
                "os": {"name": "iOs", "version": "another_version"},
            },
            format="json",
        )
        assert response.status_code == status.HTTP_201_CREATED
    ...
    def test_inactive_device_is_set_to_active_on_registration_id_change(
        self, app_user, app_api_client
    ):
        device = Device.objects.create(
            device_id="unique_id",
            registration_id="fcm_unique_token",
    ...
    def test_device_without_device_id_is_updated_on_create(self, app_user):
        ...
        response = app_api_client.post(
            ...
            data={
                ...
                "model": device.model,
                "os": {"name": device.os_name, "version": device.os_version},
            },
            format="json",
        )
        assert response.status_code == status.HTTP_201_CREATED
    ...
    def test_device_with_same_fcm_token_is_updated_on_create(
        self, app_user, app_api_client
    ):
        device = Device.objects.create(
            user=app_user,
    ...
    def test_create_device_using_existing_mobile_app(self, app_api_client, app_user):
        mobile_app = MobileApp.objects.create(
            package_name="com.test.app", package_version="1.0.0"
        )
...
class TestIdentificationTaskAnnotationsApi:
    ...
    def test_confidence_label_sets_obj_confidence(
        self,
        api_client,
        endpoint,
        common_post_data,
        ...
        response = api_client.post(endpoint, data=post_data, format="json")
        assert response.status_code == status.HTTP_201_CREATED
    ...
    def test_is_executive_sets_decision_level_executive(
        self,
        api_client,
        endpoint,
    ...
    def test_is_flagged_representation(
        ...
        response = api_client.get(endpoint + f"{obj.pk}/", format="json")
    ...
    def test_is_flagged_sets_status_to_flagged(
        self,
        api_client,
        endpoint,
        common_post_data,
    ...
    def test_empty_observation_flags(
        self, api_client, endpoint, common_post_data, with_add_permission
    ):
        post_data = common_post_data
        ...
        assert not response.data["observation_flags"]["is_favourite"]
        assert response.data["observation_flags"]["is_visible"]
    ...
    def test_is_visible_sets_status_to_public(
        self,
        api_client,
        endpoint,
        common_post_data,
        ...
        post_data = common_post_data
        post_data["observation_flags"] = {"is_visible": is_visible}
    ...
    @pytest.mark.parametrize("is_favourite", [True, False])
    def test_is_favourite(
        self, api_client, endpoint, common_post_data, with_add_permission, is_favourite
    ):
        post_data = common_post_data
        post_data["observation_flags"] = {"is_favourite": is_favourite}
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                pytest.lazy_fixture("taxon_root"),
                "male",
                False,
                False,
    ...
    def test_characteristics(
        ...
        if taxon:
            ...
            post_data.pop("classification", None)

        post_data["characteristics"] = (
            {"sex": sex, "is_blood_fed": is_blood_fed, "is_gravid": is_gravid}
            if sex
            else None
        )
    ...
    @pytest.mark.parametrize("pre_assign", [True, False])
    def test_POST_always_saves_validation_complete_True(
        self,
        pre_assign,
        api_client,
        endpoint,
    ...
    def test_classification_null_shoud_set_status_to_hidden(
        self, api_client, endpoint, common_post_data, with_add_permission
    ):
        post_data = common_post_data
        post_data["classification"] = None
        # NOTE: force is_visible. Should set to hidden anyways.
        post_data["observation_flags"] = {"is_visible": True}
...
class TestIdentificationTaskReviewApi:
    ...
    def test_agree_review(
        ...
        response = api_client.post(endpoint, data={"action": "agree"}, format="json")
        assert response.status_code == status.HTTP_201_CREATED
    ...
    def test_overwrite_and_existing_review_with_characteristics_None(
        self,
        api_client,
        endpoint,
        user_with_role_reviewer,
        ...
        assert review.sex is None
        assert review.is_blood_fed is None
        assert review.is_gravid is None

        identification_task.refresh_from_db()
        assert identification_task.sex is None
        assert identification_task.is_blood_fed is None
        assert identification_task.is_gravid is None
...
class TestFixesApi:
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                None,
                None,
                timezone.datetime(2024, 1, 1, tzinfo=timezone.utc),
                Point(0.1, 0.1, srid=4326),
                timezone.datetime(2024, 1, 1, tzinfo=timezone.utc),
            ),
            (
                Point(10.0, 20.0, srid=4326),
                timezone.datetime(2024, 1, 1, tzinfo=timezone.utc),
                timezone.datetime(2024, 1, 1, tzinfo=timezone.utc),
                Point(0.1, 0.1, srid=4326),
                timezone.datetime(2024, 1, 1, tzinfo=timezone.utc),
            ...
            (
                Point(10.0, 20.0, srid=4326),
                timezone.datetime(2024, 1, 2, tzinfo=timezone.utc),
                timezone.datetime(2024, 1, 1, tzinfo=timezone.utc),
                Point(10.0, 20.0, srid=4326),
                timezone.datetime(2024, 1, 2, tzinfo=timezone.utc),
            ),
    ...
    def test_user_last_location_is_updated_on_fix_creation(
        self,
        app_user: TigaUser,
        app_api_client,
        endpoint,
        user_last_location,
        user_last_location_update,
        fix_creation_time,
        expected_user_last_location,
        expected_user_last_location_update,
    ):
