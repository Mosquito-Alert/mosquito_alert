...
from mosquito_alert.stats.views import (
    workload_stats_per_user,
    workload_daily_report_input,
    workload_pending_per_user,
    workload_available_reports,
    get_user_xp_data,
)
...
urlpatterns = [
    ...
    re_path(
        "clear_blocked_r/(?P<username>[\w.@+-]+)/(?P<report>[\w-]+)/",
        views.clear_blocked,
    ),
    path("clear_blocked_all/", views.clear_blocked_all),
    re_path(
        "session_update/(?P<pk>\d+)/",
        views.SessionPartialUpdateView.as_view(),
        name="session_update",
    ),
