...
def populate_workspace(apps, schema_editor):
    ...
    for country in Country.objects.all().iterator():
        country_for_workspace = country
        workspace_name = None
        geom = None
        if country.iso3_code == "RPN":
            # Rapa nui -> Chile
            ...
            workspace_name = "Saint Louis, Missouri"
            geom = country.geom
        elif country.iso3_code == "SJM":
            # Svalbard -> Norway: no need for workspace, it was not a bbox.
            continue

        w = Workspace.objects.create(
            country=country_for_workspace,
            geom=geom,
            name=workspace_name,
