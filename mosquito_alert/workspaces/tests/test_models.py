...
class TestWorkspace:
    ...
    def test_geom_can_be_null(self):
        assert Workspace._meta.get_field("geom").null

    def test_geom_can_be_blank(self):
        assert Workspace._meta.get_field("geom").blank

    def test_geom_srid_is_4326(self):
        assert Workspace._meta.get_field("geom").srid == 4326
    ...
    def test_name_can_be_null(self):
        assert Workspace._meta.get_field("name").null

    def test_name_can_be_blank(self):
        assert Workspace._meta.get_field("name").blank
    ...
    def test_geom_is_validated_on_save(self):
        ...
        buffered = country.geom.buffer(1)

        # ensure MultiPolygon
        if buffered.geom_type == "Polygon":
            buffered = MultiPolygon(buffered)

        workspace.geom = buffered  # buffer makes it invalid
    ...
    def test_multiple_workspaces_with_geom_is_allowed_for_same_country(self):
        ...
        # create second workspace with buffered geom (valid but different from country)
        buffered = country.geom.buffer(-0.1)

        # ensure MultiPolygon
        if buffered.geom_type == "Polygon":
            buffered = MultiPolygon(buffered)

        WorkspaceFactory(country=country, geom=buffered)
