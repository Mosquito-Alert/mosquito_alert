    ...
    if this_user_is_expert or this_user_is_superexpert:
        ...
        if this_user_is_expert and load_new_reports == 'T':
            if current_pending < max_pending:
                ...
                new_reports_unfiltered = Report.objects.exclude(note__icontains="#345").exclude(version_UUID__in=my_reports).exclude(version_UUID__in=adults_2014_nonstandard_validation).exclude(hide=True).exclude(photos=None).filter(type='adult').annotate(n_annotations=Count('expert_report_annotations')).filter(n_annotations__lt=max_given)
            ...
            new_reports_unfiltered = Report.objects.exclude(note__icontains="#345").exclude(version_UUID__in=my_reports).exclude(hide=True).exclude(photos__isnull=True).filter(type='adult').annotate(n_annotations=Count('expert_report_annotations')).filter(n_annotations__gte=max_given)
...
def expert_report_status(request, reports_per_page=10, version_uuid=None, linked_id=None):
    ...
    if this_user.groups.filter(Q(name='superexpert') | Q(name='movelab')).exists():
        ...
        these_reports = Report.objects.exclude(hide=True).exclude(photos__isnull=True).filter(type='adult')
        if version_uuid and version_uuid != 'na':
...
def picture_validation(request,tasks_per_page='10',visibility='visible', usr_note='', type='all'):
    ...
    new_reports_unfiltered_adults = Report.objects.exclude(type='site').exclude(note__icontains='#345').exclude(photos=None).annotate(n_annotations=Count('expert_report_annotations')).filter(n_annotations=0).exclude(version_UUID__in=adults_2014_nonstandard_validation).order_by('-creation_time')
