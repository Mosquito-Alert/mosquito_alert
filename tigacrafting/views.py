...
def expert_report_annotation(request, scroll_position='', tasks_per_page='10', load_new_reports='F', year=None, orderby='date', tiger_certainty=None, site_certainty=None, pending='all', flagged=None, flagged_others=None, hidden=None, hidden_others=None, public=None, max_pending=5, max_given=3):
    ...
    if this_user_is_expert or this_user_is_superexpert:
        ...
        if request.method == 'POST':
            ...
            flagged_others_reports = ExpertReportAnnotation.objects.exclude(report__version__UUID__in=my_reports).filter(validation_complete=True, flag=True).values('report')
            hidden_others_reports = ExpertReportAnnotation.objects.exclude(report__version__UUID__in=my_reports).filter(validation_complete=True, hide=True).values('report')
            public_others_reports = ExpertReportAnnotation.objects.exclude(report__version__UUID__in=my_reports).filter(validation_complete=True, hide=False, flag=False).values('report')
            ...
            if this_user_is_superexpert:
                ...
                n_public = all_annotations.filter(report__version_UUID__in=public_others_reports).exclude(report__version__UUID__in=flagged_others_reports).exclude(report__version__UUID__in=hidden_others_reports).count()
            ...
            if flagged_others == "flagged":
                all_annotations = all_annotations.filter(report__version__UUID__in=flagged_others_reports)
            if flagged_others == "unflagged":
                all_annotations = all_annotations.exclude(report__version__UUID__in=flagged_others_reports)
            if hidden == "hidden":
                all_annotations = all_annotations.filter(hide=True)
            if hidden == "unhidden":
                all_annotations = all_annotations.filter(hide=False)
            if hidden_others == "hidden":
                all_annotations = all_annotations.filter(report__version__UUID__in=hidden_others_reports)
            if hidden_others == "unhidden":
                all_annotations = all_annotations.exclude(report__version__UUID__in=hidden_others_reports)
            if public == "public":
                all_annotations = all_annotations.filter(report__version__UUID__in=public_others_reports).exclude(report__version__UUID__in=flagged_others_reports).exclude(report__version__UUID__in=hidden_others_reports)
            if all_annotations:
