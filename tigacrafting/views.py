...
def show_validated_photos(request, type='tiger'):
    ...
    context = {'type': type, 'validated_tasks': map(lambda x: {'deleted': x.photo.report.deleted, 'hidden': x.photo.hide, 'latest_version': x.photo.report.latest_version, 'lat': x.photo.report.lat, 'lon':  x.photo.report.lon, 'photo_link': x.photo.medium_image_(), 'validation_score': getattr(x, validation_score_dic[type])}, list(validated_tasks))}
