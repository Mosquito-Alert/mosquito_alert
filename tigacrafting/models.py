...
class IdentificationTask(LifecycleModel):
    ...
    def refresh(self, force: bool = False, commit: bool = True) -> None:
        ...
        # Photo predictions
        final_photo_prediction = self.photo_predictions.filter(is_final_prediction=True).first()
...
class PhotoPrediction(models.Model, metaclass=PhotoClassifierScoresMeta):
    ...
    is_final_prediction = models.BooleanField(default=False, help_text="Indicates if this prediction can close the identification task.")
    ...
    def save(self, *args, **kwargs):
        ...
        if self.is_final_prediction:
            # Be sure no other is_final_prediction are enabled.
            PhotoPrediction.objects.filter(
                identification_task=self.identification_task,
                is_final_prediction=True
            ).exclude(pk=self.pk).update(is_final_prediction=False)
    ...
    class Meta:
        constraints = [
            ...
            models.UniqueConstraint(
                ...
                condition=models.Q(is_final_prediction=True),
