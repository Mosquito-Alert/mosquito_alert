...
class Dsmmry(dict):
	...
	def smmry0(self):
		...
		print ' '*26, ' %13.0i(%.2f)  %13.0i(%.2f)' % ((nv0+nv2), (nv0+nv2)/len(nvIdx), (nv1+nv3), (nv1+nv3)/len(nvIdx))
		print '+++   #R<=n:  %5i(%.2f)   %13.f(%.2f)  %13.f(%.2f) ' % ((nv0+nv1), (nv0+nv1)/len(nvIdx),  nv0, nv0/len(nvIdx), nv1, nv1/len(nvIdx))
		print '+++    #R>n:  %5i(%.2f)   %13.f(%.2f)  %13.f(%.2f) ' % ((nv2+nv3), (nv2+nv3)/len(nvIdx), nv2, nv2/len(nvIdx), nv3, nv3/len(nvIdx))
		print '-' *68
	...
	def catPrior(self):
		...
		if self.p == 0:
			for k in range(self.nExCat.shape[0]):
				print '+++    cat%1i     %6.0i      %6.0i       %6.0i      %6.0i  ' % ((k+1, ) + tuple(self.nExCat[k, ]))
		else:
			for k in range(self.nExCat.shape[0]):
				print '+++    cat%1i      %6.4f      %6.4f      %6.4f      %6.4f  ' % ((k+1, ) + tuple(self.nExCat[k, ]))
		print
	...
	def getScore(self, uuid):
		...
		if uuid in self.keys():
			score = int(self.rank.index(uuid)/len(self.rank)*93) + 1
		if score > 66:
