...
class Report:
    ...
    def addLbl(self, lbl, expertId):
        if expertId in _expertIds:
            self.master = expertId
            if lbl != None:
                self.lbls = [lbl]
            elif len(set(self.lbls)) == 1:
                self.lbls = self.lbls[0:1]
    ...
    def showIt(self):
        ...
        print 'expert: %s' % self.master
        print '  lbls:', self.lbls
        ...
        with ma.Cursor(dbConn) as crsr:
            ...
            while crsr.rownumber < crsr.rowcount:
                ...
                if rprtId not in dSmmry[uuid].rprtDict.keys():
                    dSmmry[uuid].rprtDict[rprtId] = Report(rprtId, iRow['type'])

                # get report label
                rprtLbl = dSmmry[uuid].rprtDict[rprtId].getLbl(iRow)
                # add report label
                dSmmry[uuid].rprtDict[rprtId].addLbl(rprtLbl, iRow['expert_id'])
