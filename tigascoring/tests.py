...
class ScoringTestCase(TestCase):
    ...
    def test_profile_first_of_season(self):
        ...
        self.assertEqual(Award.objects.filter(category__id=1).count(), 1) #Only one award given for two qualifyable reports of the same profile
    ...
    def test_profile_first_of_day(self):
        ...
        report_1_user_2.save()
        # only user_1 should have any awards
        self.assertEqual(Award.objects.filter(category__id=2).count(),1)  # Only one award given for two qualifyable reports of the same profile
        a = Award.objects.filter(category__id=1).first()
        self.assertEqual(a.given_to, user_1)  # Should have been given to user_1
